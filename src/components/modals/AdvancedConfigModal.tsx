...
import DocumentUploadModal, { UploadedDocument } from '../upload/DocumentUploadModal';
import { AdvancedConfig } from '../../hooks/useAdvancedConfig';
...
const AdvancedConfigModal: React.FC<AdvancedConfigModalProps> = ({
  ...
  // Ensure we operate on the uploadedDocuments state (safe array)
  ...
  const initialConfigFromExtracted = {
    ...
    powerMW: (extractedData as any)?.powerRequirement ?? (config?.powerMW ?? 0),
    voltage: (extractedData as any)?.voltage ?? (config?.voltage ?? 0),
  };

  // Helper to render sizes safely
  function prettySizeKb(size?: number | null) {
    const numeric = Number(size ?? 0);
    return `${(numeric / 1024).toFixed(1)} KB`;
  }

  const handleDocumentUpload = (documents: UploadedDocument[]) => {
    // Append uploaded docs into state
    setUploadedDocuments(prev => [...prev, ...documents]);

    // Extract AI suggestions from the uploaded documents (guard missing aiSuggestions)
    const newSuggestions = documents.flatMap(doc => doc.aiSuggestions ?? []);
    setAiSuggestions(prev => [...prev, ...newSuggestions]);

    // Auto-fill form fields from extracted data if present (guard that extractedData may be missing)
    if (documents.length > 0) {
      const docExtracted = documents[0].extractedData ?? {};
      setConfig({
        ...config,
        projectReference: (docExtracted as any)?.projectName ?? config.projectReference,
        customerName: (docExtracted as any)?.customerName ?? config.customerName,
        siteLocation: (docExtracted as any)?.location ?? config.siteLocation,
        powerMW: (docExtracted as any)?.powerRequirement ?? config.powerMW,
        voltage: (docExtracted as any)?.voltage ?? config.voltage,
      });
                  ...
                  <div className="space-y-1">
                    {uploadedDocuments.map((doc, index) => (
                      <div key={index} className="text-xs bg-white p-2 rounded border">
                        ...
                        <p className="text-gray-500">{prettySizeKb(doc.size)}</p>
                      </div>
                    ))}
                  </div>
                </div>
